...
-- PARTNER role users can see their linked partner (future: add partner_id to profiles)
-- For now, all authenticated can read active partners
...
-- =============================================================================
-- stundenkonto (Hours Ledger) - Check if table exists first
-- =============================================================================

DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'stundenkonto') THEN
    ...
    EXECUTE 'CREATE POLICY "stundenkonto_select_own"
    ...
    EXECUTE 'CREATE POLICY "stundenkonto_select_management"
      ON stundenkonto FOR SELECT
      TO authenticated
      USING (is_management())';

    -- Management can manage
    EXECUTE 'CREATE POLICY "stundenkonto_insert"
      ON stundenkonto FOR INSERT
      TO authenticated
      WITH CHECK (is_management())';

    EXECUTE 'CREATE POLICY "stundenkonto_update"
      ON stundenkonto FOR UPDATE
      TO authenticated
      USING (is_management())
      WITH CHECK (is_management())';

    EXECUTE 'CREATE POLICY "stundenkonto_delete"
...
-- =============================================================================
-- Additional tables - Update if they exist and have EDITOR references
-- =============================================================================

-- stuecke, szenen, rollen (plays)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'stuecke') THEN
    ...
    EXECUTE 'CREATE POLICY "stuecke_insert" ON stuecke FOR INSERT TO authenticated WITH CHECK (is_management())';
    EXECUTE 'CREATE POLICY "stuecke_update" ON stuecke FOR UPDATE TO authenticated USING (is_management())';
    EXECUTE 'CREATE POLICY "stuecke_delete" ON stuecke FOR DELETE TO authenticated USING (is_admin())';
  END IF;
END $$;
...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'raeume') THEN
    ...
    EXECUTE 'CREATE POLICY "raeume_insert" ON raeume FOR INSERT TO authenticated WITH CHECK (is_management())';
    EXECUTE 'CREATE POLICY "raeume_update" ON raeume FOR UPDATE TO authenticated USING (is_management())';
    EXECUTE 'CREATE POLICY "raeume_delete" ON raeume FOR DELETE TO authenticated USING (is_admin())';
  END IF;
END $$;
...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'ressourcen') THEN
    ...
    EXECUTE 'CREATE POLICY "ressourcen_insert" ON ressourcen FOR INSERT TO authenticated WITH CHECK (is_management())';
    EXECUTE 'CREATE POLICY "ressourcen_update" ON ressourcen FOR UPDATE TO authenticated USING (is_management())';
    EXECUTE 'CREATE POLICY "ressourcen_delete" ON ressourcen FOR DELETE TO authenticated USING (is_admin())';
  END IF;
END $$;
...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'auffuehrung_templates') THEN
    ...
    EXECUTE 'CREATE POLICY "auffuehrung_templates_insert" ON auffuehrung_templates FOR INSERT TO authenticated WITH CHECK (is_management())';
    EXECUTE 'CREATE POLICY "auffuehrung_templates_update" ON auffuehrung_templates FOR UPDATE TO authenticated USING (is_management())';
    EXECUTE 'CREATE POLICY "auffuehrung_templates_delete" ON auffuehrung_templates FOR DELETE TO authenticated USING (is_admin())';
  END IF;
END $$;
